...
pitch: "4 scattered feedback channels (Zendesk, WhatsApp, Play Store, App Store) - 2,020 customer verbatims classified across 27 business-specific themes - for $1 in API calls. Built with Python and Claude Haiku. Runs monthly from a single command, with checkpoints so an interruption doesn't cost a re-run."
...
The point isn't that AI can classify customer feedback. HelloBible now has something it didn't before: a shared vocabulary for talking about users, refreshed every month, for $1.
...
The technical details are in the accordion below.
...
That last one is the most useful lesson: **you need a second LLM to audit your first LLM's outputs**. I had Claude Opus check Haiku's classifications for consistency, and it flagged real errors I would have missed.
