...
Every published plan on YouVersion ends with a bit.ly link that redirects readers to HelloBible. The pipeline doesn't just save time on publishing - it turns reading plans into a top-of-funnel acquisition channel. YouVersion has over 600 million installs. We're systematically publishing into that audience in every language we expand to.
...
- **New language**: update translation settings and Bible version mappings (NIV for English, HSV for Dutch), set a Polly voice ID for audio, run the pipeline. The architecture doesn't change per language.
