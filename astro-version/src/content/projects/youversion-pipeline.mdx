...
After that POC, I chose to move the entire pipeline to a standalone Node.js script driven by direct API calls. Three reasons drove that decision: eliminating the dependency on third-party infrastructure, getting sub-second latency per operation, and having granular control over raw data processing that a visual workflow tool couldn't give me.
